                    ...
                    if len(name) != 0:
                        if len(bcd) != 0:
                            if Vendor.objects.filter(name=name).exists():
                                ...
                                vendor.save()
                                messages.success(
                                    request, 'Please wait for 24 hours to be validate as Vendor but you can enjoy product sa Customer till then')
...
def placeorder(request):
    if request.method == 'POST':
        ...
        order = Order.objects.create(
            order_by_id=order_by, phone=phone, location=location)
        order.save()
    ...
    for key, value in ses.items():
        ...
        data = OrderItem.objects.create(
            order_id=order.id, product_id=product_id, quantity=product_qty, total=total_amount)
        data.save()
    return render(request, 'home.html')
...
def sales(request):
    ...
    for order in orders:
        ...
        for orderitem in orderitems:
            ...
            Product.objects.filter(id=orderitem.product_id).update(
                sales=product.sales)
